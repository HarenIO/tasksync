...
function Tracker() {
  ...
  const fetchLists = async () => {
    try {
      const res = await fetch(`https://tasksync.onrender.com/api/trackers/${params.id}/lists`, {
        credentials: 'include'
      });
      if(res.status === 204){
        setLists([])
  ...
  const getTracker = async () => {
    try {
      const res = await fetch(`https://tasksync.onrender.com/api/trackers/${params.id}`, {
        credentials: 'include'
      })
      if (res.ok) {
        const data = await res.json()
  ...
  const getMembers = async () => {
    try {
      const res = await fetch(`https://tasksync.onrender.com/api/trackers/${params.id}/users`, {
        method: 'GET',
        credentials: 'include'
      })
      const data = await res.json()
